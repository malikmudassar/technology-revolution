...
{setTimeout(function(){if($('#first').val()=='yes'){set_cart_map();}},500);return paginationClick(wizard,options,state,decreaseCurrentIndexBy(state,1));}
...
$.ajax({url:form.attr('action'),type:'POST',dataType:'html',data:formdata?formdata:form.serialize(),cache:false,contentType:false,processData:false,beforeSend:function(){here.addClass('disabled');here.html(submitting);},success:function(data){here.fadeIn();here.html(text);here.removeClass('disabled');if(data=='email_sent'){notify(email_sent,'info','bottom','right');here.closest('.modal-content').find('#close_log_modal').click();}else if(data=='email_nay'){here.closest('.modal-content').find('#close_log_modal').click();notify(email_noex,'info','bottom','right');}else if(data=='email_not_sent'){here.closest('.modal-content').find('#close_log_modal').click();notify(email_fail,'info','bottom','right');}else{notify(data,'warning','bottom','right');}},error:function(e){console.log(e)}});});
